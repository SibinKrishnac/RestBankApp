...
from .serializers import UserModelSerializer, LoginSerializer, AccountModelSerializer, WithdrawSerializer,DepositSerializer,TransactionSerializer
...
class AccountCreateView(generics.GenericAPIView,mixins.CreateModelMixin):
    ...
    def get(self,request):
        accno=Transactions.objects.last()
        if accno:
            last_accno=accno.accno
            lst=int(last_accno)+1
            accno=accno+lst
        else:
            accno="1000"
        return Response({"accno":"Account number is "+str(accno)})
    def post(self,request):
        return self.create(request)
